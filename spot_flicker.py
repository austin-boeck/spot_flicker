...
data_std = pd.rolling_std(flux_sans, kernel, center=True)
...
'''
In this section I will attempt to fit the data by using a series of
sine waves
'''
tau = 0.045
def fourier(z, *a):
    ret = a[0] * np.cos(np.pi / tau * z)
    for deg in range(1, len(a)):
        ret += a[deg] * np.cos((deg+1) * np.pi / tau * z)
    return ret        
popt, pcov = curve_fit(fourier, flux_sans, time_sans, [1.0] * 15)

#plot this fourier fit
fig = plt.figure()
ax1 = fig.add_subplot(111)
p8, = plt.plot(flux_sans, fourier(flux_sans, popt[0], popt[1], popt[2], popt[3], popt[4], popt[5], popt[6], popt[7]))
plt.show()
